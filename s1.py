...
# canales en múltiples modelos de color, conversión a escala de grises,
# binarización, cálculo de características estadísticas y visualización
# de histogramas.
# -----------------------------------------------------------------------------
...
<<<<<<< HEAD
# =============================================================================
# --- SECCIÓN 1: FUNCIONES DE PROCESAMIENTO Y VISUALIZACIÓN ---
# =============================================================================
...
def dibujar_canales_rgb(fig, r, g, b):
    fig.clear(); (ax1, ax2, ax3) = fig.subplots(1, 3); zero_channel = Image.new('L', r.size, 0)
    img_r = Image.merge('RGB', (r, zero_channel, zero_channel)); img_g = Image.merge('RGB', (zero_channel, g, zero_channel)); img_b = Image.merge('RGB', (zero_channel, zero_channel, b))
    ax1.imshow(img_r); ax1.set_title("Componente R"); ax1.axis('off')
    ax2.imshow(img_g); ax2.set_title("Componente G"); ax2.axis('off')
    ax3.imshow(img_b); ax3.set_title("Componente B"); ax3.axis('off'); fig.tight_layout()

# --- Funciones para el Modelo CMY ---
def separar_canales_cmy(imagen_pil):
    if imagen_pil.mode != 'RGB': imagen_pil = imagen_pil.convert('RGB')
    imagen_cmy_np = 255 - np.array(imagen_pil)
    c = Image.fromarray(imagen_cmy_np[:, :, 0]); m = Image.fromarray(imagen_cmy_np[:, :, 1]); y = Image.fromarray(imagen_cmy_np[:, :, 2])
    return c, m, y

def dibujar_canales_cmy(fig, c, m, y):
    fig.clear(); (ax1, ax2, ax3) = fig.subplots(1, 3); c_np, m_np, y_np = np.array(c), np.array(m), np.array(y)
    h, w = c_np.shape; white = np.full((h, w), 255, dtype=np.uint8)
    img_c = np.stack([255 - c_np, white, white], axis=-1); img_m = np.stack([white, 255 - m_np, white], axis=-1); img_y = np.stack([white, white, 255 - y_np], axis=-1)
    ...
    ax3.imshow(img_y); ax3.set_title("Componente Y (Yellow)"); ax3.axis('off'); fig.tight_layout()

# --- Funciones para el Modelo HSV ---
def separar_canales_hsv(imagen_pil):
    if imagen_pil.mode != 'RGB': imagen_pil = imagen_pil.convert('RGB')
    imagen_bgr_np = cv2.cvtColor(np.array(imagen_pil), cv2.COLOR_RGB2BGR)
    imagen_hsv_np = cv2.cvtColor(imagen_bgr_np, cv2.COLOR_BGR2HSV)
    h, s, v = cv2.split(imagen_hsv_np); return h, s, v
=======
...
def dibujar_canales_rgb(fig, r, g, b):
    ...
    ax3.axis('off')
    fig.tight_layout()
>>>>>>> 592e144 (Actualizacion de  s1.py)

def dibujar_canales_hsv(fig, h, s, v):
    fig.clear(); (ax1, ax2, ax3) = fig.subplots(1, 3)
    ...
    ax3.imshow(v, cmap='gray'); ax3.set_title("Componente V (Value)"); ax3.axis('off'); fig.tight_layout()

# --- Funciones para el Modelo YIQ ---
def separar_canales_yiq(imagen_pil):
    if imagen_pil.mode != 'RGB': imagen_pil = imagen_pil.convert('RGB')
    rgb_array = np.array(imagen_pil) / 255.0; yiq_array = np.zeros_like(rgb_array)
    for i in range(rgb_array.shape[0]):
        for j in range(rgb_array.shape[1]): yiq_array[i, j] = colorsys.rgb_to_yiq(*rgb_array[i, j])
...
def dibujar_canales_yiq(fig, y, i, q):
    ...
    ax3.imshow(q, cmap='PiYG'); ax3.set_title("Componente Q (Crominancia)"); ax3.axis('off'); fig.tight_layout()
...
def dibujar_canales_hsi(fig, h, s, i):
    ...
    ax1.imshow(h, cmap='hsv'); ax1.set_title("Componente H (Hue)"); ax1.axis('off')
    ax2.imshow(s, cmap='plasma'); ax2.set_title("Componente S (Saturation)"); ax2.axis('off')
    ax3.imshow(i, cmap='gray'); ax3.set_title("Componente I (Intensity)"); ax3.axis('off'); fig.tight_layout()
...
>>>>>>> 592e144 (Actualizacion de  s1.py)
    ...
    ax.imshow(gris_cv, cmap='gray'); ax.set_title("Imagen en Escala de Grises"); ax.axis('off'); return gris_cv
...
<<<<<<< HEAD
    ...
    ax.imshow(binaria, cmap='gray'); ax.set_title(f"Imagen Binarizada (Umbral = {umbral})"); ax.axis('off'); return binaria

# --- Funciones para Características Estadísticas ---
def calcular_caracteristicas_escala_grises(canal_np):
    datos = np.asarray(canal_np).flatten()
    histograma, _ = np.histogram(datos, bins=256, range=(0, 257))
    if histograma.sum() == 0: return None
    prob = histograma / histograma.sum()
    energia = np.sum(prob ** 2)
    entropia = -np.sum([p * math.log2(p) for p in prob if p > 0])
    return {'Energía': energia, 'Entropía': entropia, 'Asimetría': skew(datos), 'Media': np.mean(datos), 'Varianza': np.var(datos)}
=======
    ax.clear()
...
<<<<<<< HEAD
    ...
    ax_hb.plot(hb, color='blue'); ax_hb.set_title("Histograma B"); ax_hb.grid(True, alpha=0.3); fig.tight_layout(pad=0.5)
...
def dibujar_histograma_canales_cmy(fig, c, m, y):
    ...
    ax_hy.plot(hy, color='orange'); ax_hy.set_title("Histograma Y"); ax_hy.grid(True, alpha=0.3); fig.tight_layout(pad=0.5)
...
def dibujar_histograma_canales_hsv(fig, h, s, v):
    ...
    ax_hv.plot(hv, color='black'); ax_hv.set_title("Histograma V"); ax_hv.grid(True, alpha=0.3); fig.tight_layout(pad=0.5)
...
def dibujar_histograma_canales_yiq(fig, y, i, q):
    ...
    ax_hq.plot(hq, color='purple'); ax_hq.set_title("Histograma Q"); ax_hq.grid(True, alpha=0.3); fig.tight_layout(pad=0.5)
...
def dibujar_histograma_canales_hsi(fig, h, s, i):
    ...
    ax_hh.plot(hh, color='red'); ax_hh.set_title("Histograma H"); ax_hh.grid(True, alpha=0.3)
    ax_hs.plot(hs, color='gray'); ax_hs.set_title("Histograma S"); ax_hs.grid(True, alpha=0.3)
    ax_hi.plot(hi, color='black'); ax_hi.set_title("Histograma I"); ax_hi.grid(True, alpha=0.3); fig.tight_layout(pad=0.5)

def dibujar_histograma_escala_grises(fig, imagen_gris_cv):
    fig.clear(); ax_img, ax_hist = fig.subplots(1, 2)
    ...
    ax_hist.set_xlabel("Intensidad"); ax_hist.set_ylabel("Frecuencia"); fig.tight_layout()
...
def dibujar_histograma_binaria(fig, imagen_binaria_cv):
    ...
    ax_hist.set_xlabel("Intensidad (0=Negro, 255=Blanco)"); ax_hist.set_ylabel("Cantidad de Píxeles"); ax_hist.grid(True, alpha=0.3); fig.tight_layout()
=======
    fig.clear()
...
<<<<<<< HEAD
        self.root = root; self.root.title("Procesador de Imágenes Digitales")
        ...
        control_frame = ttk.Frame(root, padding="10"); control_frame.pack(side=tk.TOP, fill=tk.X)
        color_model_frame = ttk.LabelFrame(control_frame, text="Modelos de Color", padding=5); color_model_frame.pack(side=tk.LEFT, padx=5, fill=tk.Y)
        ...
        single_channel_frame = ttk.LabelFrame(control_frame, text="Procesos", padding=5); single_channel_frame.pack(side=tk.LEFT, padx=5, fill=tk.Y)
        ...
        analysis_frame = ttk.LabelFrame(control_frame, text="Análisis", padding=5); analysis_frame.pack(side=tk.LEFT, padx=5, fill=tk.Y)
        ttk.Button(analysis_frame, text="Histograma", command=self.mostrar_histograma).pack(fill=tk.X, pady=2)
        ttk.Button(analysis_frame, text="Características", command=self.mostrar_caracteristicas).pack(fill=tk.X, pady=2)
        action_frame = ttk.Frame(control_frame); action_frame.pack(side=tk.RIGHT, padx=5)
    ...
    def _reset_states(self):
        self.grayscale_image_cv = None; self.binary_image_cv = None; self.current_rgb_channels = None
        self.current_cmy_channels = None; self.current_hsv_channels = None; self.current_yiq_channels = None
        self.current_hsi_channels = None; self.current_state = "original"
=======
        self.root = root
        ...
        # *** NUEVO: Sistema de historial de operaciones ***
        self.imagen_trabajo_actual = None  # Imagen en la que se está trabajando
        self.historial_operaciones = []  # Lista de operaciones aplicadas
...
<<<<<<< HEAD
    ...
    def mostrar_caracteristicas(self):
        if self.original_image is None: messagebox.showerror("Error", "Primero debes cargar una imagen."); return
        caracteristicas = {}; tipo_imagen = ""
        try:
            if self.current_state == "original":
                tipo_imagen = "Imagen Original (RGB)"; r, g, b = self.original_image.split()
                caracteristicas = {"Canal R": calcular_caracteristicas_escala_grises(r), "Canal G": calcular_caracteristicas_escala_grises(g), "Canal B": calcular_caracteristicas_escala_grises(b)}
            elif self.current_state == "grayscale" and self.grayscale_image_cv is not None:
                tipo_imagen = "Imagen en Escala de Grises"; stats = calcular_caracteristicas_escala_grises(self.grayscale_image_cv)
                if stats: caracteristicas = {"Escala de Grises": stats}
=======
        self.imagen_trabajo_actual = self.grayscale_image_cv.copy()
    ...
    def aplicar_etiquetado_directo(self, vecindad, invertir, dialog):
        ...
        try:
            ...
            # MODIFICADO: Usa la imagen potencialmente invertida
            ...
            # MODIFICADO: Añade 'Invertido' al historial si se usó
            ...
            # MODIFICADO: Muestra la imagen que realmente se etiquetó
    ...
    def abrir_dialogo_binarizacion(self):
        ...
        # Usamos un Spinbox para seleccionar el valor
        umbral_spinbox = ttk.Spinbox(main_frame, from_=0, to=255, 
                                      textvariable=umbral_var, width=15)
        umbral_spinbox.pack(anchor='w', pady=5)
        
        # También puedes usar una escala (slider) si prefieres:
        # umbral_scale = tk.Scale(main_frame, from_=0, to=255, orient="horizontal", 
        #                              variable=umbral_var, length=250)
        # umbral_scale.pack(anchor='w', pady=5)
    ...
    def ejecutar_binarizacion(self, umbral, dialog):
        ...
        # --- MODIFICADO: Usar la imagen de trabajo actual ---
        # Determinar qué imagen binarizar:
        # 1. La imagen de trabajo actual (resultado de AND, morfología, etc.)
        # 2. Si no hay, la imagen en escala de grises original.
        
        imagen_a_binarizar = None
        if self.imagen_trabajo_actual is not None:
            # Usamos la imagen que ya está procesada (ej. después de un AND)
            imagen_a_binarizar = self.imagen_trabajo_actual
        elif self.grayscale_image_cv is not None:
            # Usamos la imagen en grises si es el primer paso
            imagen_a_binarizar = self.grayscale_image_cv
        else:
            # Este caso no debería pasar gracias al check en aplicar_binarizacion
            messagebox.showerror("Error", "No hay imagen en escala de grises para binarizar.")
            return
        # --- FIN DE LA MODIFICACIÓN ---

        if imagen_a_binarizar is None: # Doble chequeo por si acaso
        ...
        try:
            ...
            # MODIFICADO: Pasa la imagen correcta (imagen_a_binarizar)
            self.binary_image_cv = binarizar_imagen_en_ax(imagen_a_binarizar, ax, umbral)
            
            # Actualizamos la imagen de trabajo para el siguiente paso
            self.imagen_trabajo_actual = self.binary_image_cv.copy()
            self.current_state = "binary"
            
            # MODIFICADO: NO reiniciamos el historial, lo continuamos
            # self.historial_operaciones = [] # <--- ESTA LÍNEA SE ELIMINA O COMENTA
            
            # NUEVO: Agregamos la binarización como un paso en el historial
...
<<<<<<< HEAD
                tipo_imagen = "Canales RGB"; r, g, b = self.current_rgb_channels
                caracteristicas = {"Canal R": calcular_caracteristicas_escala_grises(r), "Canal G": calcular_caracteristicas_escala_grises(g), "Canal B": calcular_caracteristicas_escala_grises(b)}
            elif self.current_state == "cmy_channels":
                tipo_imagen = "Canales CMY"; c, m, y = self.current_cmy_channels
                caracteristicas = {"Canal C": calcular_caracteristicas_escala_grises(c), "Canal M": calcular_caracteristicas_escala_grises(m), "Canal Y": calcular_caracteristicas_escala_grises(y)}
            elif self.current_state == "hsv_channels":
                tipo_imagen = "Canales HSV"; h, s, v = self.current_hsv_channels
                caracteristicas = {"Canal H": calcular_caracteristicas_escala_grises(h), "Canal S": calcular_caracteristicas_escala_grises(s), "Canal V": calcular_caracteristicas_escala_grises(v)}
            elif self.current_state == "yiq_channels":
                tipo_imagen = "Canales YIQ"; y, i, q = self.current_yiq_channels
                caracteristicas = {"Canal Y": calcular_caracteristicas_escala_grises(y), "Canal I": calcular_caracteristicas_escala_grises(i), "Canal Q": calcular_caracteristicas_escala_grises(q)}
            elif self.current_state == "hsi_channels":
                tipo_imagen = "Canales HSI"; h, s, i = self.current_hsi_channels
                caracteristicas = {"Canal H": calcular_caracteristicas_escala_grises(h), "Canal S": calcular_caracteristicas_escala_grises(s), "Canal I": calcular_caracteristicas_escala_grises(i)}
            
            caracteristicas = {k: v for k, v in caracteristicas.items() if v is not None}
            if not caracteristicas: messagebox.showerror("Error", "No se pudieron calcular las características."); return

            ventana_stats = tk.Toplevel(self.root); ventana_stats.title(f"Características de: {tipo_imagen}"); ventana_stats.geometry("400x400")
            texto_resultado = f"--- {tipo_imagen} ---\n\n"
            for canal, stats in caracteristicas.items():
                texto_resultado += f"--- {canal.upper()} ---\n"
                texto_resultado += f"  Energía:   {stats['Energía']:.6f}\n"
                texto_resultado += f"  Entropía:  {stats['Entropía']:.4f}\n"
                texto_resultado += f"  Asimetría: {stats['Asimetría']:.4f}\n"
                texto_resultado += f"  Media:     {stats['Media']:.2f}\n"
                texto_resultado += f"  Varianza:  {stats['Varianza']:.2f}\n\n"
            text_widget = tk.Text(ventana_stats, wrap=tk.WORD, font=('Courier', 10)); text_widget.pack(expand=True, fill=tk.BOTH, padx=10, pady=10)
            text_widget.insert(tk.END, texto_resultado); text_widget.config(state=tk.DISABLED)
        except Exception as e:
            messagebox.showerror("Error", f"Ocurrió un error al calcular las características: {e}")
