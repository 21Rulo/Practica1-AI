...
# canales en modelos RGB, CMY y HSV, conversión a escala de grises,
...
# (Las funciones de procesamiento y dibujo de canales se mantienen igual)
...
def dibujar_canales_hsv(fig, h, s, v):
    ...
    ax2.imshow(s, cmap='gray'); ax2.set_title("Componente S (Saturation)"); ax2.axis('off')
...
def dibujar_histograma_canales_rgb(fig, r, g, b):
    ...
    hr = cv2.calcHist([np.array(r)], [0], None, [256], [0, 256])
    hg = cv2.calcHist([np.array(g)], [0], None, [256], [0, 256])
    hb = cv2.calcHist([np.array(b)], [0], None, [256], [0, 256])
...
def dibujar_histograma_canales_cmy(fig, c, m, y):
    ...
    hc = cv2.calcHist([c_np], [0], None, [256], [0, 256])
    hm = cv2.calcHist([m_np], [0], None, [256], [0, 256])
    hy = cv2.calcHist([y_np], [0], None, [256], [0, 256])
...
def dibujar_histograma_canales_hsv(fig, h, s, v):
    ...
    ax_s.imshow(s, cmap='gray'); ax_s.set_title("Canal S"); ax_s.axis('off')
...
class MatplotlibApp:
    ...
    def aplicar_escala_de_grises(self):
        ...
        if self.original_image is None: messagebox.showerror("Error", "Primero debes cargar una imagen."); return
        self._reset_states()
        self.fig.clear()  # Se añade esta línea para limpiar el lienzo antes de dibujar
        ax = self.fig.add_subplot(111)
    ...
    def aplicar_binarizacion(self):
        ...
        if self.grayscale_image_cv is None:
            if messagebox.askyesno("Confirmación", "Se requiere una imagen en escala de grises. ¿Deseas convertirla ahora?"):
                self.aplicar_escala_de_grises()
                # Hacemos una pausa para que la conversión a grises se complete y se muestre
        ...
        self.fig.clear()  # Se añade esta línea para limpiar el lienzo antes de dibujar
        ax = self.fig.add_subplot(111)
        # Asegurarnos de que tenemos la imagen en grises para binarizar
    ...
    def mostrar_histograma(self):
        ...
        if self.current_state == "rgb_channels":
            dibujar_histograma_canales_rgb(self.fig, *self.current_rgb_channels)
        elif self.current_state == "cmy_channels":
            dibujar_histograma_canales_cmy(self.fig, *self.current_cmy_channels)
        elif self.current_state == "hsv_channels":
            dibujar_histograma_canales_hsv(self.fig, *self.current_hsv_channels)
        else:
            messagebox.showinfo("Aviso", "Mostrando histograma RGB de la imagen original.")
            self.fig.clear()
            ax_img, ax_hist = self.fig.subplots(1, 2)
            ...
            ax_hist.plot(hist_r, color='red', alpha=0.7, label='R')
            ax_hist.plot(hist_g, color='green', alpha=0.7, label='G')
            ax_hist.plot(hist_b, color='blue', alpha=0.7, label='B')
    ...
    def revertir_a_original(self):
        ...
        self._reset_states()
        self.canvas.draw()
        messagebox.showinfo("Revertido", "Mostrando la imagen original.")
    ...
    def guardar_figura(self):
        ...
        if ruta: 
            ...
            messagebox.showinfo("Éxito", "La vista actual ha sido guardada.")
